...
Shared action dimensions live in `css-variables.css`. Standard buttons and split buttons use 42px height, small buttons use 36px, and sidebar actions use 48px at the default application zoom. At viewport widths of 1600px or less, these become 38px, 32px, and 44px respectively, independently of the sidebar auto-width preference. Large dialog actions (`size="medium"`) and bottom actions use 52px height, reduced to 50px at viewport widths of 1600px or less, and retain 20px horizontal padding at both viewport sizes. Other action padding and icon spacing also compact; label typography stays unchanged. Split-button toggles are square and menus are positioned relative to the trigger. Context-specific controls embedded in fields or editor upload surfaces retain their local sizing. TinyMCE dialog footer actions consume the same large-button height through Publii-owned `editor-overrides.css`; dialog utility icons and toolbar controls keep their own dimensions. Block-editor advanced settings and link-dialog footer actions also consume the shared large-button height instead of deriving a fixed height from vertical padding.

Inputs use the shared input background, border, focus, text, disabled, readonly, and validation roles. `TextInput` separates native input type, visual size, keyboard blocking, and the boolean `invalid` state; these concerns must not share a string-based property. Fields draw one inset ring shape in two roles, `input-shadow-focus` and `input-shadow-invalid`, and the invalid ring stays visible while the field is focused. `Dropdown`, `RadioButton`, and `Switcher` each have one value contract through `value`/`v-model`; initial-state aliases such as `selected` and `checked` are not part of those contracts. The list-selection `Checkbox` remains a controlled primitive with explicit `checked`, item `value`, and callback props. `customCssClasses` is retained only as a schema-driven extension point for theme and plugin settings. `Field` accepts `normal` or `small` spacing while label layout states remain independent booleans. `ImageUpload` exposes `default` and `small` through its size contract. `ButtonDropdown` uses an explicit intent and derives icon layout from its icon prop. Its main action and menu toggle are native buttons and its list is a keyboard-navigable menu. `ActionMenu` is the overflow control for row and card actions: an icon-only native trigger that opens a keyboard-navigable menu of native menu items, with `danger` as its only item intent. `ProgressBar` uses semantic `default`, `success`, `danger`, and `warning` intents rather than color names. `ProgressOrb` is the phase-aware synchronization indicator: it takes a validated `phase`, a numeric `progress`, an independent `indeterminate` boolean, and a `message`; its disc shares the empty-state illustration surface roles, its glow, ring, and icon derive from the brand and status roles, and its motion is CSS-only and stops under reduced motion. Its `flat` appearance keeps the ring and supplied icon for compact preview and thumbnail-regeneration status panels, and stays neutral while idle. `Overlay` uses `default` or `drop-zone` appearance rather than color-named booleans. `Icon` uses its existing named size set and the `nonInteractive` boolean; pointer behavior must not be encoded in a string of properties. Its custom-class extension remains available for controlled file-type classes. `Collection` receives its column count explicitly; `CollectionCell` uses a validated visual `variant`, while row states such as the main author are booleans. Collections use their shared background and hover roles with the common border role. `Tabs` uses an explicit `vertical` or `horizontal` orientation and an independent `scrollable` capability, together with the shared text, hover, active-background, and active-text roles.
